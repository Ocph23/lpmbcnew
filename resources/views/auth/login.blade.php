...
<body class="bg-gray-50 flex items-center justify-center min-h-screen">
    ...
    <div class="bg-white shadow-md rounded-xl p-8 w-full max-w-md text-center">
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-4 text-left">
            ...
            <div>
                <input type="email" name="email" placeholder="Email" value="{{ old('email') }}"
                       class="w-full border @error('email') border-red-400 @else border-gray-300 @enderror focus:ring-2 focus:ring-blue-400 rounded-lg px-4 py-2 outline-none" required>
                @error('email')
            ...
            <div class="relative">
                <input id="password" type="password" name="password" placeholder="Password"
                       class="w-full border @error('password') border-red-400 @else border-gray-300 @enderror focus:ring-2 focus:ring-blue-400 rounded-lg px-4 py-2 outline-none" required>
        ...
        <p class="text-gray-600 text-sm mt-6">
            Belum punya akun?
            <a href="{{ route('register') }}" class="text-blue-600 hover:underline font-medium">Registrasi dahulu</a>
        </p>
    </div>
